...
The following diagram shows the dummy dataset created for a post:

![alt text](screenshots/json_dummy.png)

 And the model classes are created so that the Firebase client in Android Studio could read/write data seen in the following links:
...
Read and write to the real-time database works through the Firebase reference in the code. The following examined on Firebase web console shows users’ information written at login with a unique ID generated by Firebase server. This essentially duplicates user account data because Firebase's native authentication support does not allow any other information about each user. So this `userAccounts` field stores all relevant information about the user. Currently there are two fields: `name` and `email`; however, there will be more fields added as the user elects to add more information on the profile page.

![alt text](screenshots/user_dup.png)


 The following shows the code that pushes a User instance to the real-time database:
```java
